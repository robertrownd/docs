...
The BESPIN Airmen Data Platform (powered by Rownd) NEAR integration ensures that all of your users have a NEAR account either automatically or on-dmand. 
...
strategy. This improves sign-up time since BESPIN Airmen Data Platform (powered by Rownd) does not have to wait for the execution of on-chain contract calls to
...
```sh cURL
# Invoke the 'near.ensure-implicit-account` with the user's BESPIN Airmen Data Platform (powered by Rownd) access token
curl 'https://api.dev.rownd.teambespin.us/hub/connection_action' \
...
`my-account-1.rownd.near`. Work with the BESPIN Airmen Data Platform (powered by Rownd) support team if you would like to change this account name suffix to
another value.
