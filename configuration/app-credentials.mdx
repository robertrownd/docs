...
description: "Learn how app keys work with BESPIN Airmen Data Platform (powered by Rownd) code snippets, SDKs, and APIs."
---

When your code interacts with BESPIN Airmen Data Platform (powered by Rownd) through an SDK, JavaScript code snippet, or directly with our REST API, you'll need a set of credentials to authenticate your access to the BESPIN Airmen Data Platform (powered by Rownd) platform. At BESPIN Airmen Data Platform (powered by Rownd), we call these "app keys" and "app secrets." Every application provides a default app key, but you can create as many additional keys as you need.
...
You can revoke an app key and its associated secret at any time through the [BESPIN Airmen Data Platform (powered by Rownd) dashboard](https://https://app.dev.rownd.teambespin.us). Be careful, though! Revoking a key that's still in use will likely result in downtime for your app. Ensure you generate new app keys and update your deployments with them before revoking existing keys.
...
**App secrets** authenticate machine-to-machine communication with BESPIN Airmen Data Platform (powered by Rownd) to retrieve and update any user profile, generate sign-in (magic) links for any user, and support other more sensitive/restricted functions.

#### Working with app keys

From the BESPIN Airmen Data Platform (powered by Rownd) dashboard, select the application for which you want to generate an app key, then select **App keys** from the side navigation.
